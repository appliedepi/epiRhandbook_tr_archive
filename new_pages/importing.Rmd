...
An Automated Programming Interface (API) is commonly used to directly request data from a website. There are some APIs that require authentication (i.e. a platform with user ID and credentials) and others that anyone can access without credentials. Regardless, this exchange is most commonly done through an HTTP request. The input and output may vary depending on the type of API but the process is the same - a "Request" (often HTTP Request) followed by a "Response". 

There are a few different parameters of an *HTTP Request*, including: 
- the specific URL of API endpoint
- Verb/Method (Likely the only type you will deal with when pulling data into R for analysis is "GET")
- Headers
- Body

You will then receive a "response" in an expected similar format - including URL, HTTP status (Status 200 is what you want!), file type, size, and first bit of output. Then you need to parse the response and turn it into a workable dataframe within your R environment.

The *httr* package works quite well for working with HTTP requests in R, even for those who are less savvy with software development-speak! It requires little prior knowledge of Web APIs. In addition, some packages can help with parsing the response depending on the structure - if it's .json, you can use **jsonlite**.
...
### API Example where Authentication is NOT required (i.e. publically available online data) {-} 

Below is a simple example of an HTTP request from [The Trafford Data Lab](https://www.trafforddatalab.io/open_data_companion/#A_quick_introduction_to_APIs) which has a number of other easy-to-follow API exercises.
...
Below we retrieve a cleaned, tidy table of fast food outlets, as specified in query.

```{r, echo=F}
DT::datatable(df, rownames = FALSE, options = list(pageLength = 12, scrollX=T), class = 'white-space: nowrap' )
```


### API Example where Authentication is required - Go.Data API {-}

Below is some sample code using *httr* and *jsonlite* for connecting to the Go.Data API to pull in contact linelists.
