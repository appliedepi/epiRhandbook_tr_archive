...
`clean_variable_spelling()`|Veri sözlüğü kullanarak değerlerin yeniden kodlanması ve temizliği|**linelist**
...
Eşlik etmek isterseniz, <a href='https://github.com/epirhandbook/Epi_R_handbook/raw/master/data/case_linelists/linelist_raw.xlsx' class='download-button'>ham satır listesini indirmek için tıklayınız</a> (.xlsx dosyası olarak).  
...
linelist <- linelist %>% 
  mutate(age_years = case_when(
            ...
            TRUE                 ~ NA_real_)) # başka bir durum, eksik olarak atama
...
Bir veri çerçevesini bir temizleme sözlüğü ile temizlemek için R paketi satır listesini ve clean_variable_spelling() fonksiyonunu kullanın. linelist, RECON - the R Epidemics Consortium tarafından geliştirilen bir pakettir. [RECON](https://github.com/reconhub/linelist) - the R Epidemics konsorsiyum  
...
3. Ham satır listesini, wordlists = temizleme sözlüğü veri çerçevesini belirterek clean_variable_spelling() öğesine iletin. Yazım_vars = argümanı, sözlükteki hangi sütunun hangi sütuna (varsayılan olarak 3.) atandığını belirtmek için kullanılabilir veya sözlüğün tüm karakter ve faktör sütunlarına uygulanması için NULL olarak ayarlanabilir. Bu fonksiyonun çalışmasının uzun zaman alabileceğini unutmayın.

```{r}
linelist <- linelist %>% 
  linelist::clean_variable_spelling(
    wordlists = cleaning_dict,
    spelling_vars = "col",        # sütun adlarını içeren dict sütunu, varsayılan olarak dict'te 3. sütuna
  )
```
...
Temizleme sözlüğündeki sütun adlarınızın, temizleme komut dosyanızın bu noktasındaki adlara karşılık gelmesi gerektiğini unutmayın. Daha fazla ayrıntı için [satır listesi paketi için bu çevrimiçi referansa](https://www.repidemicsconsortium.org/linelist/reference/clean_data.html) bakabilirsiniz.
